...
def rnn_cell_forward(xt, a_prev, parameters):
    ...
    #print(np.shape(np.dot(Wax,xt))
    #print(np.shape(np.dot(Waa,a_prev))
...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    concat[: n_a, :] = a_prev     #分片的前后； ：n_a表示n_a行之前， n_a:，则表示n_a行之后
    ...
    c_next = ft * c_prev + it * cct          #element-wise muiltyply
...
def rnn_cell_backward(da_next, cache):
    ...
    dba = np.sum(dtanh*da_next, axis=1, keepdims=1)
...
$$d\tilde c^{\langle t \rangle} = dc_{next}*\Gamma_u^{\langle t \rangle}+ \Gamma_o^{\langle t \rangle} (1-\tanh(c_{next})^2) * \Gamma_u^{\langle t \rangle} * da_{next} * \tilde c^{\langle t \rangle} * (1-\tanh(\tilde c)^2) \tag{8}$$
