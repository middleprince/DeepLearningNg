...
```
    ...
    cosine similarity between receptionist and g, after neutralizing:  -2.3364129329e-17
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = np.dot(mu, bias_axis) * bias_axis / np.sum(np.power(mu, 2))
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = np.dot(e_w1, bias_axis) * bias_axis / np.sum(np.power(e_w1, 2))
    e_w2B = np.dot(e_w2, bias_axis) * bias_axis / np.sum(np.power(e_w2, 2))
...
```
    ...
    cosine_similarity(e1, gender) =  -0.141503548893
    cosine_similarity(e2, gender) =  0.287527415706
